...
document.addEventListener("click",e=>{
 ...
 if(act==="sale"){
  ...
  chosen.forEach(type=>{const amount=type==="Personnalisé"?customAmount:Number(state.prices[type]||0);if(amount>0){const saleId=state.nextId++;state.sales.push({id:saleId,userId:currentUser.id,employeeName:currentUser.name,type,amount,createdAt:d});state.transactions.push({id:state.nextId++,type:"income",category:"Tatouage",description:`Tatouage ${type} — ${currentUser.name}`,amount,createdAt:d,saleId,employeeId:currentUser.id,employeeName:currentUser.name,source:"tatouage"});state.bank+=amount;total+=amount}});
