...
function tattooPage(){const s=activeService(currentUser.id);const items=[["Petit","machine-petit.svg","Petit format"],["Moyen","machine-moyen.svg","Format moyen"],["Grand","machine-grand.svg","Grand format"],["Personnalisé","machine-personnalise.svg","Montant libre"]];const chosen=selectedTattoos||[];const total=chosen.filter(k=>k!=="Personnalisé").reduce((a,k)=>a+Number(state.prices[k]||0),0);return `${header("Tatouage","Choisissez une ou plusieurs tailles, puis validez le paiement.")}${!s?'<div class="notice">⚠ Vous devez être en service pour enregistrer une prestation.</div>':""}<section class="card"><div class="row"><h3>Nouvelle prestation</h3><span class="badge">Sélection multiple</span></div><div class="tattoo-grid">${items.map(([k,img,sub])=>`<button type="button" class="tattoo-option ${chosen.includes(k)?"selected":""}" data-tattoo="${k}" ${s?"":"disabled"}><img src="assets/${img}" alt="Machine à tatouer"><span>${k}</span><b>${k==="Personnalisé"?"Montant libre":money(state.prices[k])}</b><small>${sub}</small></button>`).join("")}</div>${chosen.length?`<div class="payment-box"><div><small>SÉLECTION</small><b>${chosen.map(esc).join(" + ")}</b></div><div><small>TOTAL</small><b>${chosen.includes("Personnalisé")?"Montant à définir":money(total)}</b></div><button type="button" class="btn primary" data-action="sale">Valider ${chosen.length>1?"les paiements":"le paiement"}</button></div>`:""}</section>`}
...
document.addEventListener("click",e=>{
 ...
 if(t){if(t.disabled)return;const k=t.dataset.tattoo;selectedTattoos=selectedTattoos||[];if(k==="Personnalisé"){selectedTattoos=selectedTattoos.includes(k)?[]:[k]}else{selectedTattoos=selectedTattoos.filter(x=>x!=="Personnalisé");selectedTattoos=selectedTattoos.includes(k)?selectedTattoos.filter(x=>x!==k):[...selectedTattoos,k]}render();return}
