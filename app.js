...
let state=load(), currentUser=null, page="home", selectedTattoos=[];
...
function tattooPage(){const s=activeService(currentUser.id);const items=[["Petit","machine-petit.svg","Petit format"],["Moyen","machine-moyen.svg","Format moyen"],["Grand","machine-grand.svg","Grand format"],["Personnalisé","machine-personnalise.svg","Montant libre"]];const chosen=selectedTattoos||[];const total=chosen.filter(k=>k!=="Personnalisé").reduce((a,k)=>a+Number(state.prices[k]||0),0);const counts=chosen.reduce((o,k)=>(o[k]=(o[k]||0)+1,o),{});const selection=Object.entries(counts).map(([k,n])=>`${esc(k)} × ${n}`).join(" + ");return `${header("Tatouage","Cliquez plusieurs fois sur une taille pour ajouter plusieurs tatouages, puis validez le paiement.")}${!s?'<div class="notice">⚠ Vous devez être en service pour enregistrer une prestation.</div>':""}<section class="card"><div class="row"><h3>Nouvelle prestation</h3><span class="badge">Sélection multiple</span></div><div class="tattoo-grid">${items.map(([k,img,sub])=>`<button type="button" class="tattoo-option ${counts[k]?"selected":""}" data-tattoo="${k}" ${s?"":"disabled"}><img src="assets/${img}" alt="Machine à tatouer"><span>${k}${counts[k]?` × ${counts[k]}`:""}</span><b>${k==="Personnalisé"?"Montant libre":money(state.prices[k])}</b><small>${sub}</small></button>`).join("")}</div>${chosen.length?`<div class="payment-box"><div><small>SÉLECTION</small><b>${selection}</b></div><div><small>TOTAL</small><b>${chosen.includes("Personnalisé")?"Montant à définir":money(total)}</b></div><div class="actions"><button type="button" class="btn" data-action="undoTattoo">Retirer le dernier</button><button type="button" class="btn danger" data-action="clearTattoos">Vider la sélection</button><button type="button" class="btn primary" data-action="sale">Valider ${chosen.length>1?"les paiements":"le paiement"}</button></div></div>`:""}</section>`}
...
document.addEventListener("click",e=>{
 ...
 if(pg){page=pg.dataset.page;selectedTattoos=[];render();return}
 ...
 if(t){if(t.disabled)return;const k=t.dataset.tattoo;selectedTattoos=selectedTattoos||[];selectedTattoos.push(k);render();return}
 const act=e.target.closest("[data-action]")?.dataset.action;
 if(act==="logout"){currentUser=null;selectedTattoos=[];render();return}
 if(act==="undoTattoo"){selectedTattoos=selectedTattoos||[];selectedTattoos.pop();render();return}
 if(act==="clearTattoos"){selectedTattoos=[];render();return}
 if(act==="service"){
 ...
 if(act==="sale"){
  ...
  const chosen=[...(selectedTattoos||[])]; if(!chosen.length)return;
  let customAmount=0;
  if(chosen.includes("Personnalisé")){customAmount=Number(prompt("Montant du tatouage en $","0"));if(customAmount<=0){alert("Montant invalide.");return}}
  const d=new Date().toISOString(); let total=0;
  chosen.forEach(type=>{const amount=type==="Personnalisé"?customAmount:Number(state.prices[type]||0);if(amount>0){const saleId=state.nextId++;state.sales.push({id:saleId,userId:currentUser.id,type,amount,createdAt:d});state.transactions.push({id:state.nextId++,type:"income",category:"Tatouage",description:`Tatouage ${type} — ${currentUser.name}`,amount,createdAt:d,saleId});state.bank+=amount;total+=amount}});
  selectedTattoos=[];save();toast(`Paiement enregistré : ${money(total)}.`);render();return
 }
 if(act==="income"){
...
function deleteTransaction(id){const t=state.transactions.find(x=>x.id===Number(id));if(!t)return;const amount=Number(t.amount)||0;state.transactions=state.transactions.filter(x=>x.id!==t.id);if(t.type==="income")state.bank-=amount;else state.bank+=amount;if(t.saleId)state.sales=state.sales.filter(x=>x.id===Number(t.saleId));state.bank=Number(state.bank)||0;save();document.getElementById("deleteTransactionModal")?.remove();render();toast("Recette supprimée et solde mis à jour.")}
function openDeleteTransactionModal(id){const t=state.transactions.find(x=>x.id===Number(id));if(!t)return;const old=document.getElementById("deleteTransactionModal");if(old)old.remove();document.body.insertAdjacentHTML("beforeend",`<div id="deleteTransactionModal" class="modal-backdrop"><div class="employee-modal card"><div class="modal-head"><div><div class="eyebrow">SAINTS TATTOO</div><h3>Supprimer cette recette ?</h3></div><button type="button" class="btn small" data-transaction-cancel>Fermer</button></div><p class="muted">La recette de ${money(t.amount)} sera retirée de la comptabilité et le solde bancaire sera mis à jour.</p><div class="modal-actions"><button type="button" class="btn" data-transaction-cancel>Annuler</button><button type="button" class="btn danger" data-delete-transaction-confirm="${t.id}">Supprimer</button></div></div></div>`)}
