...
document.addEventListener("click",e=>{
 ...
 if(act==="logout"){currentUser=null;selectedTattoos=[];tattooCounts={};render();return}
 if(act==="undoTattoo"){const last=(selectedTattoos||[]).at(-1);if(last){tattooCounts[last]=Math.max(0,(tattooCounts[last]||0)-1);if(!tattooCounts[last])delete tattooCounts[last];selectedTattoos=Object.entries(tattooCounts).flatMap(([type,n])=>Array(n).fill(type));}render();return}
 if(act==="clearTattoos"){selectedTattoos=[];tattooCounts={};render();return}
 if(act==="service"){
 ...
 if(act==="sale"){
  ...
  selectedTattoos=[];tattooCounts={};save();toast(`Paiement enregistré : ${money(total)}.`);render();return
 }
 if(act==="income"){
